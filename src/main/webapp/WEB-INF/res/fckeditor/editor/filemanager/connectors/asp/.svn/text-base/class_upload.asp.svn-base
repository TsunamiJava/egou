...
Class NetRube_Upload
	...
	Public	File, Form
	Private oSourceData
	Private nMaxSize, nErr, sAllowed, sDenied, thtmlExtensions
	...
	Public Sub SaveAs(sItem, sFileName)
		...
		' Check XSS.
		If Not IthtmlExtension( sFileExt ) Then
	...
	Private Function IthtmlExtension( sExt )
		If thtmlExtensions = "" Then
			Exit Function
		End If
		...
		oRE.IgnoreCase	= True
		oRE.Global		= True
		oRE.Pattern		= thtmlExtensions

		IthtmlExtension = oRE.Test(sExt)
