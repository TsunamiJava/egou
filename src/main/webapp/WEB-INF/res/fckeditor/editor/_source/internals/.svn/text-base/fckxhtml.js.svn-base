...
FCKXHtml.GetXHTML = function( node, includeNode, format )
{
	...
	this._NbspEntity = ( FCKConfig.ProcesthtmlEntities? 'nbsp' : '#160' ) ;
...
FCKXHtml.TagProcessors =
{
	...
	{
		var thtml = htmlNode.innerHTML ;

		// Gecko does give back the encoded html
		if ( FCKBrowserInfo.IsGecko )
			thtml = FCKTools.HTMLDecode( thtml );

		// Remove the saved urls here as the data won't be processed as nodes
		thtml = thtml.replace( /\s_fcksavedurl="[^"]*"/g, '' ) ;

		node.appendChild( FCKXHtml.XML.createTextNode( FCKXHtml._AppendSpecialItem( thtml ) ) ) ;
